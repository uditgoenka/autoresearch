...
description: Use when user types /autoresearch_probe or asks for adversarial requirement / assumption interrogation. Multi-persona probe loop that interrogates user and codebase until net-new constraints saturate, then emits ready-to-run autoresearch config.
agent: build
---
...
- `--scope <glob>` or `Scope:` — codebase glob for grounding
- `--chain <targets>` or `Chain:` — comma-separated downstream commands
- `--mode <mode>` or `Mode:` — interactive (uses `question`) or autonomous (self-answer with confidence labels)
- `--adversarial` — rotate Skeptic + Contradiction Finder + Edge-Case Hunter to the front
...
1. Read the probe workflow: `.opencode/skills/autoresearch/references/probe-workflow.md`
2. If topic, depth, or personas is missing — use `question` with batched adaptive questions per probe-workflow.md PREREQUISITE Interactive Setup section
...
5. If `--chain` is set, hand off `handoff.json` to each chained command sequentially.
