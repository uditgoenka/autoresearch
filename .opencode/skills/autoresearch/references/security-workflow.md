...
opencode run --command autoresearch/security -- --fail-on critical --iterations 10
...
jobs:
  security-audit:
    ...
    steps:
      ...
      - name: Install Autoresearch OpenCode files
        run: |
          ...
          cp -r /tmp/autoresearch/.opencode/skills/autoresearch .opencode/skills/autoresearch
          cp /tmp/autoresearch/.opencode/commands/*.md .opencode/commands/
          cp /tmp/autoresearch/.opencode/agents/docs-manager.md .opencode/agents/docs-manager.md
      ...
      - name: Run Security Audit
        ...
        run: |
          ...
          if [ "${{ github.event_name }}" = "pull_request" ]; then
            opencode run --command autoresearch/security -- --diff --fail-on critical --iterations 5
          else
            opencode run --command autoresearch/security -- --fail-on high --iterations 15
          fi
