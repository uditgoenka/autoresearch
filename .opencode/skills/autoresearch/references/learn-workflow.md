...
**TOOL AVAILABILITY:** `question` is a built-in OpenCode tool. If it is unavailable or denied in the current session, stop and explain that interactive setup cannot continue until the `question` tool is allowed. NEVER skip interactive setup because of a tool issue.
...
4. **Strategy by count:**
   ...
   - **4-6 files:** Spawn 2-3 `explore` subagents
   - **7+ files:** Spawn 4-5 `explore` subagents (max 5)
...
7. **`explore` subagent output contract:**
   ```
...
## Phase 4: Generate — Spawn `docs-manager` Subagent

**CRITICAL:** Spawn the `docs-manager` subagent via the `task` tool with all gathered context. Do not wait for user input.
...
2. **Doc reading results** (from Phase 3 `explore` subagents — Update mode only)
...
1. Check script exists: `[ -f "$HOME/.config/opencode/scripts/validate-docs.cjs" ]`
2. If exists: run `node $HOME/.config/opencode/scripts/validate-docs.cjs docs/`
