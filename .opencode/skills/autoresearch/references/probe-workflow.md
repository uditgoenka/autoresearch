...
- User invokes `/autoresearch_probe`
...
/autoresearch_probe --depth deep --personas 8
Topic: Event-driven order management system
```

## PREREQUISITE: Interactive Setup (when invoked without topic)

**CRITICAL — BLOCKING PREREQUISITE:** If `/autoresearch_probe` is invoked without a topic description, you MUST use `question` to gather context BEFORE proceeding to ANY phase. DO NOT skip this step. DO NOT jump to Phase 1 without completing interactive setup.
...
You MUST call `question` with ALL selected questions in a SINGLE batched call:
...
**IMPORTANT:** Batch ALL selected questions into a SINGLE `question` call. NEVER ask one at a time — users need full context to make informed decisions together. If `question` only supports one question per call, include all questions in a single call with numbered headers.
...
/autoresearch_probe
  ...
  ├── Phase 6:  Answer Capture      (single question call)
...
- If user selects "Cancel" in any `question` → exit cleanly: "Probe cancelled. Run `/autoresearch_probe` again when ready." Output directory is NOT created — no partial files.
...
Issue a single batched `question` call with the ≤5 synthesized questions. Each question is presented with its originating persona label so the user understands the interrogation angle.
...
/autoresearch_probe --depth standard
...
/autoresearch_probe --chain predict
...
/autoresearch_probe --chain scenario,debug,fix
